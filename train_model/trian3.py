...
class ResNetTrainer:
    ...
    def get_dynamic_alpha(self, contrastive_loss, classification_loss, epoch, epochs):
        ...
        """
        # 平滑更新损失
        self.smoothed_contrastive_loss = self.beta * self.smoothed_contrastive_loss + (1 - self.beta) * contrastive_loss.item()
        self.smoothed_classification_loss = self.beta * self.smoothed_classification_loss + (1 - self.beta) * classification_loss.item()

        # 计算损失变化率
        contrastive_loss_change = abs(self.smoothed_contrastive_loss - contrastive_loss.item()) / max(self.smoothed_contrastive_loss, 1e-8)
        classification_loss_change = abs(self.smoothed_classification_loss - classification_loss.item()) / max(self.smoothed_classification_loss, 1e-8)
...
def get_dataloader(dataset_name, batch_size=4, train=True):
    if dataset_name == 'cifar10':
        ...
        transform = transforms.Compose([...])  # CIFAR-100类似的变换
        dataset = datasets.CIFAR100(root='./data', train=train, download=True, transform=transform)
    elif dataset_name == 'mnist':
        transform = transforms.Compose([...])  # MNIST的变换
        dataset = datasets.MNIST(root='./data', train=train, download=True, transform=transform)
    elif dataset_name == 'cinic10':
        transform = transforms.Compose([...])  # CINIC-10的变换
        dataset = datasets.ImageFolder(root='./cinic-10', transform=transform)
    else:
...
def main():
    model = resnet18(num_classes=10)  # 替换为根据数据集不同的类别数量
    optimizer = optim.AdamW(model.parameters(), lr=3e-5, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10, eta_min=1e-7)

    # 使用 CIFAR-10 作为示例数据集
    train_loader = get_dataloader('cifar10', batch_size=4, train=True)
    val_loader = get_dataloader('cifar10', batch_size=4, train=False)

    trainer = ResNetTrainer(model, optimizer, scheduler, patience=20)
    trainer.train(train_loader, val_loader=val_loader, epochs=100)
    trainer.save_model('resnet_pretrained_optimized4.pth')
